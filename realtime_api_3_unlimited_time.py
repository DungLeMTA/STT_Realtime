...
class Listener:
    ...
    def listen(self, queue, prev_audio, start):
    ...
    def run(self, queue, pre, start):
        thread = threading.Thread(target=self.listen, args=(queue, pre, start), daemon=False)
        thread.start()
...
class SpeechRecognitionEngine:
    ...
    def run(self, action):
        self.listener_save.run(self.audio_q,self.prev,self.start)
